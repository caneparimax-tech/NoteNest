...
if (isAjaxRequest() && isset($_GET['action']) && $_GET['action'] === 'get_notes') {
    ...
    if ($sectionID) {
        try {
             $stmt = $dbconnect->prepare("
                SELECT Notes.*, Students.FName, Students.LName 
                FROM Notes 
    ...
    if (isset($_SESSION['CourseID']) && isset($_SESSION['SectionID'])) {
        $courseID = $_SESSION['CourseID'];
        $sectionID = $_SESSION['SectionID'];
        
        // Fetch course name etc.
        $stmt = $dbconnect->prepare("SELECT CourseName FROM Courses WHERE CourseID = :courseID");
        $stmt->bindParam(':courseID', $courseID, PDO::PARAM_INT);
        $stmt->execute();
        $courseName = $stmt->fetch(PDO::FETCH_ASSOC)['CourseName'] ?? 'No course name'; 
...
// Generate the URL with URL encoding and HTML escaping
$url = "adminCourseSectionInformation.php?sectionId=" . htmlspecialchars($sectionID) .
       "&courseCode=" . urlencode($courseCode) .
       "&courseName=" . urlencode($courseName) .
       "&instructorName=" . urlencode($instructorName) .
...
<body>
    ...
    <div class="topbar">
<a href="<?= $url ?>" class="breadcrumb-link">Section Information</a><span class="arrow">></span>
    <span class="breadcrumb-current"><?php echo htmlspecialchars($courseName); ?></span>
...
<ul>
    ...
    <li><a href="instructorEnrollment.php" class="enrollment"><i class="material-icons">school</i> Enrollment</a></li>
...
<a href="deleteViewNotes.php?noteID=${encodeURIComponent(note.NoteID)}&title=${encodeURIComponent(note.NoteTitle)}&noteDesc=${encodeURIComponent(note.NoteDesc)}&date=${encodeURIComponent(note.UploadDate)}&author=${encodeURIComponent(note.FName + ' ' + note.LName)}&filePath=${encodeURIComponent(note.FilePath)}" class="section-card-link">
