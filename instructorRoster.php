...
// Uncomment and update the condition and redirection to login.php accordingly.
...
$courseName = $_GET['courseName'] ?? 'Unknown Course';
...
if ($sectionId) {
    ...
    ");
    if (false === $stmt_students) {
        die("Prepare failed: " . htmlspecialchars($dbconnect->errorInfo()[2]));
    }
    $stmt_students->bindParam(':sectionId', $sectionId, PDO::PARAM_INT);
    if (!$stmt_students->execute()) {
        die("Execute failed: " . htmlspecialchars($stmt_students->errorInfo()[2]));
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'removeStudents') {
    ...
    try {
        ...
        $stmt_unenroll = $dbconnect->prepare("
            DELETE FROM Enrollment 
            WHERE SectionID = :sectionId AND StudentID = :studentId
        ");

        foreach ($studentIds as $studentId) {
            $stmt_unenroll->bindParam(':sectionId', $sectionId, PDO::PARAM_INT);
            $stmt_unenroll->bindParam(':studentId', $studentId, PDO::PARAM_INT);
...
.breadcrumb-current {
    font-weight: bold; /* Example: make the current section bold */
    padding-left: 0 !important;
}
.breadcrumb-link .arrow {
    color: #fff;
    font-size: 30px; /* Adjust the size of the arrow as needed */
    /* You do not need padding here if the space comes from .breadcrumb-link */
