...
if (isset($_POST['delete']) && $_POST['delete'] == 'yes') {
    ...
    try {
        ...
        if ($stmt->execute([$noteID])) {
            header("Location: pendingNotes.php");
            exit;
        } else {
...
if (isset($_POST['verify']) && $_POST['verify'] == 'yes') {
    ...
    // Use try-catch block to catch any PDO exceptions
    try {
        // SQL to update the Verified status of a note
        $sql = "UPDATE Notes SET Verified = 1 WHERE NoteID = ?";
        $stmt = $dbconnect->prepare($sql);
        ...
        if ($stmt->execute([$noteID])) {
            ...
            // If execute returns false, log the error info array
            $errorInfo = $stmt->errorInfo();
            $error = "Verification failed: " . $errorInfo[2];
            error_log("Verification failed: " . print_r($errorInfo, true));
        }
    } catch (PDOException $e) {
        // Catch any PDO exception and log the error
        $error = "PDO Exception on verification: " . $e->getMessage();
        error_log("PDO Exception on verification: " . $e->getMessage());
    }
}
  ...
  <div class="right-sidebar" id="rightSidebar">
    ...
    <div class="right-sidebar-content">
      ...
      <p>Date Published: <?php echo $date; ?></p>
        ...
        <button type="submit" class="verify-button" onclick="return confirmVerification()">Verify Note</button>
    </form>
...
function confirmVerification() {
    return confirm('Are you sure you want to verify this note?');
}
