...
<body>
    ...
    <div class="topbar">
         <span><a href="allCourses.php" style="color: #fff; text-decoration: none; font-size: 25px;;">All Courses</a> > <?php echo htmlspecialchars($course['CourseCode']); ?> Sections</span>
...
<div class="section-card" data-section-id="<?php echo htmlspecialchars($section['SectionID']); ?>" onclick="location.href='adminCourseSectionInformation.php?sectionId=<?php echo htmlspecialchars($section['SectionID']); ?>&courseCode=<?php echo urlencode($course['CourseCode']); ?>&courseName=<?php echo urlencode($course['CourseName']); ?>&instructorName=<?php echo urlencode($instructorName); ?>'">
    ...
    <p>Instructor: <?php echo htmlspecialchars(getInstructorName($section['InstructorID'], $dbconnect)); ?></p>
...
<?php endforeach; ?>
  ...
  <div class="modal-content">
    ...
    <form id="addSectionForm">
        ...
        <input type="button" class="add-btn" id="submitAddSection" value="Add Section">
    </form>
  </div>
</div>
<script>
...
function addSectionCard(sectionId, instructorName) {
  // Find the 'Add Course Section' button
  ...
  sectionCard.innerHTML = `
    ...
    <div class="dropdown">
      <div class="dropdown-content">
        <a href="#" class="remove-btn" onclick="removeSection(${sectionId});">Remove</a>
      </div>
    </div>
  `;
  
  // Insert the new section card before the 'Add Course Section' button
  addButton.parentNode.insertBefore(sectionCard, addButton);

  // Hide the 'no sections' message if it's visible
  var noSectionsMsg = document.querySelector('.no-sections-message');
  if (noSectionsMsg) {
